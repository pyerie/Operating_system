...
void read_boot_sector(unsigned char drive, struct fat_BS_t* boot_sector_data){
...
void kernel_main(uint32_t magic, multiboot_info_t* mbd){
	...
	uint64_t rcs = 0;
	int ring;
	struct fat_BS_t boot_sector_data;
	read_boot_sector(0, &boot_sector_data);
	unsigned int total_sectors = boot_sector_data.total_sectors_16;
	unsigned short fat_size = boot_sector_data.table_size_16;
	unsigned short root_dir_sectors = ((boot_sector_data.root_entry_count * 32) + (boot_sector_data.bytes_per_sector - 1)) / boot_sector_data.bytes_per_sector;
	unsigned short first_data_sector = boot_sector_data.reserved_sector_count + (boot_sector_data.table_count * fat_size) + root_dir_sectors;
	unsigned short first_fat_sector = boot_sector_data.reserved_sector_count;
	unsigned short total_data_sectors = total_sectors - (boot_sector_data.reserved_sector_count + (boot_sector_data.table_count * fat_size) + root_dir_sectors);
	unsigned short total_clusters = total_data_sectors / boot_sector_data.sectors_per_cluster;
