...
struct dir_entry
{
	...
	unsigned short high_bits_of_first_cluster;
	unsigned short last_modified_time;
	unsigned short last_modified_date;
	unsigned short low_bits_of_first_cluster;
...
void ata_lba_read(uint32_t lba, uint8_t sector_count, unsigned char* buffer){
	uint32_t lba_temp = lba;
	//uint16_t word_count = (512 / 2) * sector_count;
	unsigned int to_read = sector_count * 512;
	unsigned int read = 0;
	outportb(ATA_DRIVE_HEAD_PORT, 0xe0 | ((lba >> 24) & 0x0f));
	outportb(ATA_SECTOR_COUNT_PORT, sector_count);
	outportb(ATA_SECTOR_NUMBER_PORT, (uint8_t) lba);
	outportb(ATA_CYLINDER_LOW_PORT, (uint8_t) (lba >> 8));
	outportb(ATA_CYLINDER_HIGH_PORT, (uint8_t) (lba >> 16));
	outportb(ATA_COMMAND_PORT, 0x20);
	while (!(inportb(ATA_COMMAND_PORT) & ATA_STATUS_DRQ));
	insw(ATA_DATA_PORT, buffer, to_read / 2); // insw reads in words; 1 word = 2 bytes
//	for (unsigned int i = 0; i < to_read; i++){
//		buffer[i] = inportb(ATA_DATA_PORT);
//		read++;
//	}
//	printf("Read: %d\n", read);
...
void read_dir(struct boot_sector boot_sector_data, unsigned int root_cluster, unsigned char first_data_sector, unsigned char* out){
	//int first_sector_of_cluster = ((root_cluster - 2) * boot_sector_data.bpb.sectors_per_cluster) + first_data_cluster;
	uint32_t lba_address = (root_cluster - 2) * boot_sector_data.bpb.sectors_per_cluster + first_data_sector;
	ata_lba_read(lba_address, (uint8_t)boot_sector_data.bpb.sectors_per_cluster, out);
}
void kernel_main(uint32_t magic, multiboot_info_t* mbd){
	...
	// Read entries
	unsigned char dir_read[boot_sector_data.bpb.sectors_per_cluster * boot_sector_data.bpb.bytes_per_sector];
	unsigned int root_cluster = boot_sector_data.ebpb.root_cluster;
	read_dir(boot_sector, root_cluster, first_data_sector, dir_read);
	if (root_cluster < 2){
		printf("Invalid root cluster number!\n");
	} else {
		struct dir_entry *entry = (struct dir_entry*) dir_read;
		unsigned int entry_count = sizeof(dir_read) / 32;
		for (unsigned int i = 0; i < entry_count; i++){
			// Implement the entry reading function
